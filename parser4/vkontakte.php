...
class Vkontakte
{
 ...
 print_r($response);
     echo '<hr>';
        ...
        //exec("curl -X POST -F \"photo=@C:\\pic.jpg\" \"$uploadURL\"", $output);
		...
		if (version_compare(phpversion(), '5.5.0', '<'))
			curl_setopt($ch, CURLOPT_POSTFIELDS, array('photo' => '@' . $fullServerPathToImage));
		else
			curl_setopt($ch, CURLOPT_POSTFIELDS, array('photo' => new \CURLFile($fullServerPathToImage)));
		$output = curl_exec($ch);
		curl_close($ch);
 ...
 */
        ...
        $response = $this->api('wall.post',
            [
                ...
                'from_group' => 1,
                'message' => "$text",
                'attachments' => "{$response['response'][0]['id']}", // uploaded image is passed as attachment
               // 'attachments' => "http://news24-7.ru/img/1386641.jpg", // uploaded image is passed as attachment
